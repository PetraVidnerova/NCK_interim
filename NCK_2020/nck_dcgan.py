...
def make_generator_model2():
  ...
  model.add(layers.Dense(gf_dim * 6 * 8)) # (4096)
  model.add(layers.Reshape((6, 8, 64))) # (8, 8, 64)
  ...
  # conv3
  #model.add(Resizing(w*75//4, w//4)) #16
  model.add(Resize2D(w*75//4, w//4))
  ...
  # conv4
  #model.add(Resizing(w*0.75//2, w//2)) #32
  model.add(Resize2D(w*0.75//2, w//2))
  #model.add(layers.Conv2DTranspose(gf_dim * 1, (5, 5), strides=(2, 2), padding='same'))
  model.add(layers.Conv2D(64, (3, 3), strides=(1, 1), padding='same'))
  model.add(layers.Activation('elu'))
  model.add(layers.Conv2D(64, (3, 3), strides=(1, 1), padding='same'))
  ...
  # conv5
  #model.add(Resizing(w*0.75//1, w//1)) #64
  model.add(Resize2D(w*0.75//1, w//1))
  #model.add(layers.Conv2DTranspose(gf_dim * 1, (5, 5), strides=(2, 2), padding='same'))
  model.add(layers.Conv2D(64, (3, 3), strides=(1, 1), padding='same'))
  model.add(layers.Activation('elu'))
  model.add(layers.Conv2D(64, (3, 3), strides=(1, 1), padding='same'))
...
# The BEGAN Discriminator
def make_discriminator_model2():
  f = 64
  h = 1
  model = tf.keras.Sequential()
  model.add(layers.InputLayer(input_shape=(48, 64, 1)))
  model.add(layers.Conv2D(f, (3, 3), strides=(1, 1), padding='same'))
  model.add(layers.Activation('elu'))
  ...
  # conv3_enc
  model.add(layers.Conv2D(2*f, (3, 3), strides=(1, 1), padding='same'))
  ...
  # conv4_enc
  model.add(layers.Conv2D(3*f, (3, 3), strides=(1, 1), padding='same'))
  model.add(layers.AveragePooling2D(strides=1, padding='same'))
  model.add(layers.Conv2D(3*f, (3, 3), strides=(1, 1), padding='same'))
  model.add(layers.AveragePooling2D(strides=1, padding='same'))
  model.add(layers.Conv2D(3*f, (3, 3), strides=(1, 1), padding='same'))
  ...
  # conv5_enc
  model.add(layers.Conv2D(4*f, (3, 3), strides=(1, 1), padding='same'))
  model.add(layers.AveragePooling2D(strides=1, padding='same'))
  model.add(layers.Conv2D(4*f, (3, 3), strides=(1, 1), padding='same'))
  model.add(layers.AveragePooling2D(strides=1, padding='same'))
  model.add(layers.Conv2D(4*f, (3, 3), strides=(1, 1), padding='same'))
...
def train(dataset, epochs):
  ...
  while epoch < EPOCHS:
    ...
    if epoch % 5 == 0 or epoch == 1:
      ...
      print(msg)
      with open("dcgan_images/fid.txt", "a+") as fidfile:
...
def generate_and_save_images(model, epoch, test_input):
  ...
  image.save(f"dcgan_images/fake_{epoch*1000}.jpg")
...
def save_real_images(dataset):
  ...
  image = np.squeeze(image)
  image = Image.fromarray(image)
  image.save("dcgan_images/reals.jpg")
...
f = open("dcgan_images/fid.txt", "w"); f.close()
...
anim_file = 'dcgan_images/progress.gif'

with imageio.get_writer(anim_file, mode='I') as writer:
  filenames = glob.glob('dcgan_images/fake*.jpg')

...
def train(dataset, epochs):
  ...
  while epoch < EPOCHS:
    ...
    if epoch % 5 == 0 or epoch == 1:
      ...
      print(msg)
      with open("lsganc_images/fid.txt", "a+") as fidfile:
...
def generate_and_save_images(model, epoch, test_input):
  ...
  image.save(f"lsganc_images/fake_{epoch*1000}.jpg")
...
def save_real_images(dataset):
  ...
  image = np.squeeze(image)
  image = Image.fromarray(image)
  image.save("lsganc_images/reals.jpg")
...
f = open("lsganc_images/fid.txt", "w"); f.close()
...
anim_file = 'lsganc_images/progress.gif'

with imageio.get_writer(anim_file, mode='I') as writer:
  filenames = glob.glob('lsganc_images/fake*.jpg')
